...
bot.onText(/\/queues/, async (msg) => {
    ...
    try {
        ...
        await bot.sendMessage(msg.chat.id, "Oops, sth went wrong..." /* DefaultAlert */);
    }
});
            ...
            if (queueForm.name.length === 0) {
                ...
                try {
                    ...
                    if (e.response.status === 403) {
                        await bot.sendMessage(msg.chat.id, "Queue with this name already exists" /* QueueExist */);
                        queueForm.name = ``;
                        queueForm.numberOfStudents = null;
                    }
                    else {
        ...
        if (type) {
            try {
                ...
                if (e?.response?.status === 403) {
                    ...
                    await bot.answerCallbackQuery(msg.id, {
                        text: "Oops, sth went wrong..." /* DefaultAlert */, show_alert: true,
                    });
                }
            }
        }
